...
//购买nft_market
import nft_market_abi from "@/abi/nft_market.json"

//查询竞拍列表
import action_abi from "@/abi/action_abi.json"

//输入竞拍价格json文件
import acution_buy_abi from "@/abi/acution_buy.json"

//给竞拍授权
import acution_appr_abi from "@/abi/acution_appr_abi.json"

//usdt合约地址
import usdt_blance_abi from "@/abi/usdt_blance.json"

//获取wand平台币合约
import wand_abi from "@/abi/wand_abi.json"
...
export default {
  install(Vue, options) {
    ...
    //nft_market列表购买
    var nft_market_addr = '0xEA56aBB3D3BC992241744cFA814FAf5b822f1712'

    //竞拍列表
    var action_addr = "0x1e2d7Eca42dcBcC9F3528697319BBd811E97a12f"

    //竞拍价格输入
    var acution_buy_addr = '0x5d11A45e11e7b6bc075514db46f65c9f9520af6a'

    //竞拍授权
    var acution_appr_addr = '0x97Bf0e9FF043E5BA7339d1c4afDd486BFeB182c4'

    //usdt余额合约
    var usdt_blance_addr = '0x97Bf0e9FF043E5BA7339d1c4afDd486BFeB182c4'

    //获取wand合约
    var wand_abi_addr = "0xc95aCfd6Aef9a229eEe6943e78C570a7D1C076cF"
    ...
    Vue.prototype.getActionList = function(data){
      console.log(data,'进入')
      var action = new web3.eth.Contract(action_abi,action_addr);
      action.methods.bids(data).call().then(res=>{
        console.log('竞拍信息=',res)
      });  
    }
    Vue.prototype.buyNftMarket = function(data,index){
      var buynft = new web3.eth.Contract(nft_market_abi,nft_market_addr);
      // var num = Number(Math.pow(10,18) * data);
      var num = Number(data);
      console.log('num=',num)
      try {
        buynft.methods.buyItem('0xd0d108709A14FEcf4B7e1172488c198932c142A8',index).send({ from: WalletAddress,value:num }).then(res=>{
    ...
    //查询wand余额
    Vue.prototype.getBlancewand = function (){
      var wandappr = new web3.eth.Contract(wand_abi, wand_abi_addr);
      try {
        wandappr.methods.balanceOf(WalletAddress).call().then(resBalance => {
          console.log('获取wand余额',resBalance);
          this.$store.commit('user/commitwandBalance',(Number(resBalance)/Math.pow(10,18)).toFixed(2))
        })
      } catch (error) {
          console.log('查询wand异常',error)
      } 
    }

    //查询usdt余额
    Vue.prototype.getUSDTblance = function (){
      var usdtappr = new web3.eth.Contract(usdt_blance_abi, usdt_blance_addr);
      try {
        usdtappr.methods.balanceOf(WalletAddress).call().then(resBalance => {
          console.log('获取usdt余额',resBalance);
          this.$store.commit('user/commitusdtBalance',Number(resBalance)/Math.pow(10,18))
        })
      } catch (error) {
          console.log('查询usdt异常',error)
      }
    ...
    //用户竞拍输入
    Vue.prototype.inSetAcutionPrice = async function(index,data){
      console.log('下标+内容=',index,data)
      var appr = new web3.eth.Contract(acution_appr_abi,acution_appr_addr);
      var setAcution = new web3.eth.Contract(acution_buy_abi, acution_buy_addr);
      await appr.methods.approve(acution_buy_addr,data).send({ from: WalletAddress }).then(res=>{
        console.log('结果=',res)
        if(res.status){
          setAcution.methods.doBid(index,data).send({ from: WalletAddress }).then(res=>{
            console.log('结果二=',res)
          })
        }
      })
